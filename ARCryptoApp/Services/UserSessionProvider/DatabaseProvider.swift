...
class DatabaseManager: DatabaseProvider {
    ...
    init() {
        ...
        NetworkMonitor.shared.addStatusUpdateHandler { [weak self] status in
            self?.reconnectDatabaseIfNeeded(status: status)
        }
    }

    private func reconnectDatabaseIfNeeded(status : NetworkMonitor.Status) {
        switch status {
        case .unreachable:
            database.goOffline()
        case .cellular:
        ...
        print("Status:", NetworkMonitor.shared.status)
        print("Reachable:", NetworkMonitor.shared.isReachable)
    }
}
